...
<div class="mx-auto max-w-2xl rounded-[2rem] border border-violet-200 bg-white/90 p-8 shadow-sm">
    <div class="mb-6">
        ...
        <p class="text-sm text-slate-600">Catat pembayaran untuk booking layanan salon.</p>
    </div>
    ...
    <form action="{{ route('payments.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="number" name="amount" value="{{ old('amount') }}" step="0.01" required class="mt-2 w-full rounded-2xl border border-violet-200 bg-violet-50 px-4 py-3 text-slate-900 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-200" />
        </div>

        <div>
            <label class="block text-sm font-medium text-slate-700">Metode Pembayaran</label>
            <input type="text" name="payment_method" value="{{ old('payment_method') }}" required class="mt-2 w-full rounded-2xl border border-violet-200 bg-violet-50 px-4 py-3 text-slate-900 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-200" />
        </div>
        ...
        <div>
            ...
            <input type="date" name="paid_at" value="{{ old('paid_at') }}" class="mt-2 w-full rounded-2xl border border-violet-200 bg-violet-50 px-4 py-3 text-slate-900 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-200" />
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <button type="submit" class="rounded-2xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white hover:bg-violet-700">Simpan Pembayaran</button>
        </div>
    </form>
</div>
@endsection